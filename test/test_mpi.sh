...
time mpirun -n $num_of_nodes ./d2 -i data/icip14_data/total.d2 -p 2 -n $batch_size -d 3,3 -s 8,8 --phase_only 0 --clusters 10 -o centroids.d2 > /dev/null


echo '[$num_of_nodes processes] Clustering images: n=2000 and k=10 with #1 phase'
time mpirun -n $num_of_nodes ./d2 -i data/icip14_data/total.d2 -p 2 -n $batch_size -d 3,3 -s 8,8 --phase_only 1 --clusters 10 -o centroids.d2 > /dev/null


echo '[$num_of_nodes processes] Clustering images: n=2000 and k=10 with both phases'
time mpirun -n $num_of_nodes ./d2 -i data/icip14_data/total.d2 -p 2 -n $batch_size -d 3,3 -s 8,8 --clusters 10 -o centroids.d2 > /dev/null
