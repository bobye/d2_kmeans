...
time mpirun -n $num_of_nodes ./d2 -i data/synthetic_data/${filename}.d2 -p 1 -n ${batch_size} -d ${dim_of_vec} -s ${size_of_bag} --clusters $k --max_iter $max_iter 


## The following command is the same as before 
## but no triangle inequality is used to prone 

# time mpirun -n $num_of_nodes ./d2 -i data/synthetic_data/${filename}.d2 -p 1 -n ${batch_size} -d ${dim_of_vec} -s ${size_of_bag} --clusters $k --max_iter $max_iter --non_triangle
