...
int d2_centroid_sphADMM(mph *p_data,
			...
			__OUT__ sph *c) {
  long i,j;
  int iter, nIter = 50, admm, admmIter = 10;
...
#pragma omp parallel for reduction(+:fval)
  ...
  for (iter = 0; iter < nIter; ++iter) {
	...
	_D2_FUNC(rsum2)(str, 1, L + i*str, q + label[i]*str);
      } 
      _D2_FUNC(cnorm)(str, num_of_labels, q, NULL);
      _D2_CBLAS_FUNC(copy)(num_of_labels*str, q, 1, c->p_w,1);
      /*
      for (i=0; i < num_of_labels; ++i) {
	d2_qp(str, label_count[i], q + label[i]*str, c->p_w + label[i]*str, c->p_w + label[i]*str);
      }
      */
      /* step 3, update L */
      for (i=0; i<size; ++i) {
