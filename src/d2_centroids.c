...
int d2_allocate_work_sphBregman(sph *ph, var_sphBregman * var_phwork) {
	...
	(SCALAR *) malloc (ph->col * sizeof(SCALAR));
      var_phwork->Zr=
	(SCALAR *) malloc (ph->str * ph->size * sizeof(SCALAR));
      var_phwork->Y = 
...
int d2_centroid_sphBregman(mph *p_data, // data
  ...
  // Indeed, we only need to reinitialize for entries whose label are changed  
  ...
  for (iter=0; iter < max_niter; ++iter) {
    ...
    res = _D2_CBLAS_FUNC(dot)(str*col, C, 1, X, 1) / size;
